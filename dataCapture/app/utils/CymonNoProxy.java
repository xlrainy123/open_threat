...
public class CymonNoProxy{
	private static final String authorization = "Bearer 00bd61a12044a62d5a88ff5ed4643e392a99bbd1";
	private static final String contentType = "application/json";
//	private static final String ip_report = "https://api.cymon.io/v2/ioc/search/ip/%s?proxies=%s";
...
//			client = HttpUtils.getHttpClient(true);
		...
		for (int i = 1; i < 2 ; i++) {
			Request request = new Request.Builder()
					.url(String.format(ip_report_malware, ip))
...
//		OkHttpClient client = null;
		...
		for (int i = 1; i < 2 ; i++) {
			Request request = new Request.Builder()
					.url(String.format(ip_report_domain, ip))
					.header("Authorization", authorization)
					.header("Content_Type", contentType)
					.build();
			OkHttpClient client = builder.build();
...
//		OkHttpClient client = null;
		...
		for (int i = 1; i < 2 ; i++) {
			Request request = new Request.Builder()
					.url(String.format(ip_report_event, ip))
					.header("Authorization", authorization)
					.header("Content_Type", contentType)
					.build();
			OkHttpClient client = builder.build();
...
//		OkHttpClient client = null;
		...
		for (int i = 1; i < 2 ; i++) {
			Request request = new Request.Builder()
					.url(String.format(ip_report_url, ip))
					.header("Authorization", authorization)
					.header("Content_Type", contentType)
					.build();
			OkHttpClient client = builder.build();
